export declare const kID: unique symbol;
export declare const kVal: unique symbol;
export declare const kSession: unique symbol;
export declare const kStreams: unique symbol;
export declare const kTimers: unique symbol;
export declare const kSocket: unique symbol;
export declare const kState: unique symbol;
export declare const kType: unique symbol;
export declare const kVersion: unique symbol;
export declare const kServer: unique symbol;
export declare const kConns: unique symbol;
export declare const kClientState: unique symbol;
export declare const kNextStreamID: unique symbol;
export declare const kNextPacketNumber: unique symbol;
export declare const kACKHandler: unique symbol;
export declare const kIntervalCheck: unique symbol;
export declare const kDelayAckTimer: unique symbol;
export declare const kUnackedPackets: unique symbol;
export declare const kFC: unique symbol;
export declare const kHS: unique symbol;
export declare const kRTT: unique symbol;
